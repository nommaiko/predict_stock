...
stock_name = st.text_input("例：APPL,FB,SFTBY（大文字・小文字どちらでも可)","AAPL")
...
st.header(stock_name_full + " 2022年1月１日から現在までの価格（USD）")
st.write(df_stock)

st.header(stock_name_full + " 終値都14年間平均（USD）")
...
st.header(stock_name_full + "1か月後の予測をしよう(USD)")
def stock_predict():
